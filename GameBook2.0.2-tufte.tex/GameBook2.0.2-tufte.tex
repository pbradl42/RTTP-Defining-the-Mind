...
1892&Krafft-Ebing publishes \emph{Psychopathia Sexualis} \fullref{kraft-ebingricardvon.psychopathiesexualis1894}\\
...
\section{Brief History of Linguistics}
...
Symptoms---and of course we are dealing now with psychical (or psychogenic) symptoms and psychical illness---are acts detrimental, or at least useless, to the subject's life as a whole, often complained by him as unwelcome and bringing unpleasure or suffering to him. (\emph{Introductory Lectures}, p. 445)
\end{quote}
...
\multicolumn{3}{ | p{13.8cm} | }{\textbf{Psychophysiologic autonomic and visceral disorders}} \\ \hline
...
\multicolumn{3}{ | p{13.8cm} | }{\textbf{Psychoneurotic Disorders}} \\ \hline
...
\multicolumn{3}{ | p{13.8cm} | }{\textbf{Personality Disorders}} \\ \hline
--x&\multicolumn{2}{ | p{12.8cm} | }{Disorders of psychogenic origin or without clearly defined tangible cause of structural change} \\ \hline
...
\multicolumn{3}{ | p{13.8cm} | }{\textbf{Transient Situational Personality Disorders}} \\ \hline
...
The Program Committee reviews all proposals for both individual reports and symposia. The guidelines for proposing a report are included in the official \fullref{callforpapersandsymposiua}.
...
\section{Outline of Game Sessions}
\label{outlineofgamesessions}

 \begin{longtable}[!t]{ | p{2cm} | p{1cm} | p{11cm} | }
\hline

\textbf{Year---Location}&\textbf{Session}&\textbf{Activities} \\ \hline
...
\subsection{Nomenclature Committee}
...
\end{enumerate}
 ...
 %\captionof{InfoBox}{Sample schedule for Board of Directors meeting\label{sample:bod}}
...
\subsubsection{Psychoanalytic Treatment}
...
\subsubsection{Behaviorism as a treatment in psychiatry}
...
If you follow the news, this story will probably feel familiar:

\begin{figure*}\includegraphics{../images/scientificReasoningSnipit.jpg}\caption{Scan of Washington Post article "Heart attacks, traffic linked", 10/24/2004}\label{fig:scientificreasoning1}\end{figure*}

We see this kind of thing all the time in the media --- there is even a website (http:\slash \slash kill-or-cure.herokuapp.com\slash ) that tracks all the things that the British Tabloid \emph{The Daily Mail} has said either causes or cures cancer.
...
Heart attacks and traffic are linked
\end{quote}

as the central claim---the conclusion, as it were,---of the study, what would count as evidence in support? Against? 

What, exactly, does it mean to be `linked' anyway? How are they linked? Does increasing one cause increases in the other? Another example is presented in the sidebar.
\begin{marginfigure}
 \begin{center}
...
\end{center}
 \caption{"Dogged crying, lower IQ linked", Washington Post, 11/1/2004}
\label{fig: BlueCross}
\end{marginfigure}

http:\slash \slash www.washingtonpost.com\slash wp-dyn\slash articles\slash A14670--2004Oct31.html?noredirect=on
...
\end{apatextbox}
From https:\slash \slash www.ncbi.nlm.nih.gov\slash pubmed\slash 15499048.

In both of these cases, journalists use the word `linked.' The scientific version of the second makes different claim: that crying ``may be a marker'' for cognitive deficits during childhood. This word change---from `marker' to `link'---has implications that demand investigation.

The former---that `dogged crying' is linked to lower IQ---suggests a causal claim: that if you could just stop the child from crying, the child's IQ would remain normal. The second---that crying may be a marker---suggests that crying is an early sign of an underlying problem that manifests as lower IQ at a later date. The first implies a causal relationship, the second, something more complicated.

This process of drawing upon information, whether acquired through simple observation or through experiment, to answer questions is the crux of the activity of scientific reasoning. The answers to these questions are what scientists refer to as hypotheses.\marginnote{A hypothesis is often defined as an 'educated guess'. This overly-simple definition is really only suitable as a starting point. A hypothesis is a conjecture about the way some phenomenon in the world is or behaves. It might be about the origin of the universe, or the mechanisms of memory, but in either case, the hypothesis claims that something is a certain way. If we discover that the thing is not the way the hypothesis claimed, we have disconfirmed the hypothesis.} As one answers these questions one is testing various hypotheses. 
...
Constructing good hypotheses and good questions to test hypothesis can be challenging, especially in psychology and psychiatry. In this section, we'll introduce some basic termniology regarding scientific reasoning and discuss a few model studies---those we call `paradigms'---that have influenced how science has progressed.

\subsubsection{The original ideal: Newton's experimentum crucis}
\label{theoriginalideal:newtonsexperimentumcrucis}

The concept of an `experiment' in science is actually relatively new. 

As you may remember from \fullref{prehistoryofpsychology:empiricismaboutthemind}, we attribute the birth of the scientific method to Francis Bacon. His study of heat in the \emph{Novum Organon} established a model of scientific reasoning that had to do with exhaustive observations:

\begin{enumerate}
\item Catalog all the instances of the thing (``Table of existence and presence'', pg XX)

\item Catalog all the instances of things similar to those in (1), but lacking heat (``Table of Absence, pg xx'')

\item Catalog all the instances of things that do not have heat but could (``Table of degrees or comparative instances'', pg. XX)

\item Finally, catalog all the instances of things that heat cannot be (Table of exclusions)

\end{enumerate}

Extrapolations{\ldots}

Newton first started his experiments on light and prisms in the early 1670's, reporting his initial findings to the Royal Society in 1671. ~\citep{Newton:1671uw} It is actually his \emph{second} letter on this topic, published in 1672, that is most frequently cited as the primary source. ~\citep{Newton:1672tm}. Newton does \textbf{not} use the term `experiment' in either of these letters. He calls his investigations `Queries'.\footnote{Actually, he spelled it ``Serie's of Quere's'', but that doesn't really matter in this case.}

There's a sense in which experiments can be seen as finding a way to put a phenomenon in one table or another. Take the bending of light through a prism. This phenomenon was known long before Newton, and the spreading of `white' light into a rainbow was well understood. What was not know was whether the colors of the rainbow were existent in the light and `revealed' by the prism, or whether they were in the prism and `added' to the white light as it passed through. In short, is the prism breaking light up, or acting as a colored filter?

Light does one other thing when it passes through a prism: it refracts. The light coming out of a prism does not continue on the same path as it was traveling when it went in---it splits off at a determinate angle corresponding to the color. Again, this was well known before Newton's experiment. 

So here's the first step in Newton's logic: suppose we're following Bacon's method, and we want to know of the angle of refraction and the color of light are the same thing or two different things. We make a table of all the instances of refraction and light

As the angle of refractionf and color are \emph{always} linked, they

Newton's \emph{experimentum crucis}, or `critical experiment' sought to 

The distinction between long and short term memory seems relatively clear cut. But there are tasks that draw upon both. Take the following memory test. You will see 12 3-letter words presented for one second each. Try to remember as many as you can. You will be asked to repeat this experiment three times.

Was the above test a test of short-term or long-term memory? A whole list of 12 3-letter words exceeds most people's capacity for short-term memory. But many people are able to rehearse the last few words over the time interval between the completion of the list and typing their responses. So for those items one is probably relying on short-term memory. But for the other items, you were using long-term memory.
Psychologists who have studied people's memory on lists such as these have observed that both the first words and last words on the list tend to be remembered better than those in the middle. They have given different names to these effects:
 • The recency effect is the finding that the last few items in a list are usually better remembered than items in the middle.
 • The primacy effect is the finding that the first few items in a list are usually better remembered than items in the middle.
A common explanation of the primacy effect is that the first items in the list are mentally rehearsed and this results in a stronger trace in long-term memory. Together these two effects give rise to the learning curve, which looks like:
...
If H, then P. Not P. Therefore, Not H.
Notice that there is an important difference between these two arguments. The hint is: one of them is inductive, and the other of them is deductive. Can you say which? Click below for the answer.

The first argument is invalid and the second is valid. Why? In the first case, it is possible for the premises to be true and for the conclusion to nonetheless be false. P might turn out to be true and it may be a deductive consequence of H, but H might still be false. For example, I could argue `If you are not over 21, you cannot drink beer. I am not drinking beer, therefore, I am not over 21.' The premises are true, but the conclusion false. I am both over 21 and drinking coffee. The argument, therefore, is not truth-preserving and not valid.
Let us take an example from the history of science: the positions of the planets can very easily be predicted with Ptolemy's 2nd Century geocentric model of the solar system, with Kepler's heliocentric geometrical model, or with Newton's gravitational account. We now know that none of these are technically true descriptions of planetary motion (they all have well-known exceptions or make positively false claims); still each makes a number of true predictions. Each of them can make produce premises like `If my theory is true, then Venus will rise in the morning and in the evening', and `Venus rises in the morning and in the evening', but the joining these true premises into an argument can not ensure that the theory is true. In fact, none of them are.
The first argument is a fallacy known as the ``affirming the consequent''. The second premise (P) is the second term, or consequent, of the first premise (If H, then P). The argument states the first premise (If H then P) and then affirms its consequent (P), in order to conclude H. Although arguments of this sort superficially resemble truth-preserving arguments like the second deductive argument above, we have seen that it fails to ensure a true the conclusion when the premises are true. We will call these arguments - arguments that superficially resemble deductive arguments but are not truth-preserving - invalid arguments.
Not so for the second argument. If the hypothesis deductively entails P and P turns out to be false, then something must be wrong with the hypothesis. As we will see, there are typically other ways to shift blame from H onto other aspects of the prediction, but nonetheless you should be able to see this simple logical point now.
The arguments above embody what is known as the ``hypothetico-deductive'' model of scientific reasoning. The hypothesis entails a prediction and the prediction is then checked against the world. If the prediction turns out to be true, the hypothesis is confirmed (though not with certainty). If it turns out to be false, the hypothesis is falsified with certainty. This simple model requires some elaboration before it can be taken as an adequate, if abstract, description of scientific reasoning.
In order to truly appreciate the power and limitations of the hypothetical-deductive method, we must take a brief sojourn into the basics of argumentation.
...
An auxiliary hypothesis is a hypothesis about the methods, techniques, materials, or instruments used in an experiment that ensures the formulation of a prediction from a hypothesis.

 
Print
Font + -
The fallible character of human knowledge
As the previous module should have convinced you, there is no infallible route to truth in science. In the face of apparently falsifying evidence, a scientist can always propose to reject another assumption or to fault the experimental procedure that produced the evidence. Sometimes it is entirely reasonable for a scientist to simply recognize that a certain bit of evidence does seem to count against his or her hypothesis, and leave the problem for future generations. Moreover, a highly confirmed theory supported by many correct predictions can eventually be rejected as either new evidence arises which falsifies them or as new alternative plausible hypotheses emerge that equally explain the same phenomena.
One response to realizing that science is not so clear cut is skepticism --- deny that we can never know the truth about the world and give up the quest to try to know. This response, however, seems extreme given the manifest success of science in revealing a great deal about the operation of the world. Even if some of what scientists claim to know now turns out to be false, it is highly unlikely that all the claims of modern science will turn out to be radically mistaken. A more plausible response is fallibilism. This recognizes that any of the claims to knowledge at a time might later be withdrawn as counter evidence or better hypotheses are developed, but also that overall the hypotheses of today are a major advance of those of a previous era. To gain an appreciation for the fallible character of science, consider two examples from the history of science, one of a theory that seemed well ensconced until it tumbled quickly at the end of the 18th century, and another which experienced ups and downs over two centuries before finally giving way.

We acquire knowledge of the world primarily through our senses. As humans, we rely most heavily, but clearly not exclusively, on vision. Seeing the world seems very straight-forward. We open our eyes and the world impresses itself upon our consciousness. But, as we will see, it is more complex than it seems at first. Our ability to see is, in part, learned. One aspect of `successful' seeing is the ability to recognize things when one encounters them again. Recognition requires the ability to categorize. Seeing, in this robust sense, also implies the ability to organize the categories into a taxonomy or classification system.
Although much scientific research goes beyond mere observation to posit causal processes and mechanisms behind what is observed, observational research plays a vital role in the development of scientific knowledge--- both in delineating the phenomenon of study and in validating hypothesis and predictions. When conducting observational research, the question of how we organize observations into categories, as well as how we organize categories into taxonomies, plays a central role. In addition, we must pay careful attention to the notion of a `variable', and how we measure and estimate the values of variables. In this module, we focus on issues of this sort:

\subsection{Observation research}
\label{observationresearch}

\subsubsection{Margaret Mead}
\label{margaretmead}

Margaret mead under scrutiny
Coming of Age in Samoa came under intense scrutiny after a half-century as an anthropological classic. In books published in 1983 and 1998, Derek Freeman charged that Mead's work suffered from too many biasing preconceptions, too much gullibility, and too little data. He went so far as to re-interview some of the same individuals, now at the other end of their lives, and reported an important recantation. Though Mead had many defenders, the damage to her reputation sent a message to ambitious young anthropologists that accurate and thorough observation is the bedrock upon which even the cleverest accounts must stand. That is, they must become skilled ethnographers.*
*Ethnographers seek to describe and understand each culture from its own perspective, using techniques that emphasize naturalistic settings, the meaning and context of activities and practices, and qualitative analysis. Such techniques as participant observation and in-depth interviews yield detailed records (written field-notes, audio and video recordings, etc.) that must be distilled and interpreted to obtain a usable account of the culture or some aspect of the culture. Today these techniques are increasingly being applied to the study of microcultures within the ethnologist's own culture. For example, Bonnie Nardi moved from studying the culture of Samoa as an outsider to studying corporate microcultures in Silicon Valley as more of an insider. She informally discusses her work at http:\slash \slash argus-acia.com\slash people\slash current\_profile.html.

\subsubsection{Roger Brown}
\label{rogerbrown}

Roger Brown and his research team at Harvard University in the 1960s visited the homes of ``Adam,'' ``Eve,'' and ``Sarah'' every two weeks during the period they were first acquiring English. The team audiotaped and transcribed everything the children and their mothers said, resulting in three corpora of such high quality that they are still in use. (Although teenagers talk somewhat differently today than in the 1960s, 2-year-olds then and now talk about juice, dogs, hats, more, allgone, etc.) Especially in the earliest stages, children omit a lot and parents sometimes fill in what is missing. Here is one of Brown's examples:
Child: Throw Daddy Mother: Throw it to Daddy
...
Child: What John will read? Mother: What will John read?
Superficially the child just mixed up the word order a bit, but Brown's group proposed a much deeper analysis in terms of Chomsky's generative grammar, which at the time, was a new and controversial approach to linguistics.
Observation

Brown's child subjects produced sentences like ``What John will read?'' and ``Why kitty can't stand up?''.

Chomsky's theory
The earliest versions of generative grammar emphasized the idea (adapted from Chomsky's mentor, Zellig Harris) that a sentence should be viewed as the product of a series of transformations. The following sequence leaves out Chomsky's linguistic symbols but gets across the general idea:
John will read what? BASE QUESTION (atypical word order for a question) What John will read? PREPOSING TRANSFORMATION: moves wh-word (what) to the front.  What will John read? TRANSPOSING TRANSFORMATION: reverses the subject (John) and the auxiliary verb (will).
...
Brown's use of Chomsky's theory to analyze the observation
...
Sources: Brown, R. (1968). The development of wh questions in child speech. Journal of Verbal Learning and Verbal Behavior, 7, 277--290.  Brown, R. (1970). Psycholinguistics. NY: The Free Press. Chomsky, N. Syntactic Structures. The Hague: Mouton. Chomsky, N. (1964). Aspects of the Theory of Syntax. Cambridge, MA: MIT Press.  Peccei, J. S. (1994), Child Language. London: Routledge.
For more on child language corpora, click here.

What all observational research has in common
 • Observational researchers always observe some type of phenomena. (obviously)
 • Observational researchers make some sort of record and analyze data obtained from it.
 • Observational researchers do not manipulate variables.
Reasons to do observational research
 • Description (atheoretical research): To obtain a general characterization of the behavior of a particular individual or group in a specified domain and setting.
 • Exploratory research (pretheoretical research): To develop hypotheses and\slash or tentative theories. The researcher obtains a characterization of behavior, but with the goal not only of description but also of explanation.
 • Confirmation or falsification (theoretical research): To test hypotheses --- which may come directly from exploratory research or have a more complex history. For example, a tested hypothesis may be obtained by revising a hypothesis that was suggested by exploratory research but failed to be confirmed. (Note: If the hypothesis concerns a causal claim, observation alone will not provide strong test; if possible, an experiment should be done.)
Two types of observational research
 • Naturalistic observation: unobtrusive, passive observation in a natural environment. It is important to minimize any impact of observer bias. For examples, the observers may be research assistants who are blind to the goals of the study.
 • Participant observation: the observer interacts with those being observed, typically within a natural environment or context. It is important to minimize reactivity --- that is, the influence of the observer's presence on the behavior of those being observed. For example, a participant observer can seek to achieve some degree of unobtrusiveness and passivity by avoiding leadership roles and making written records when out of sight.
Risks in observational research
Two of the major risks of observational research are observer bias and reactivity.
As we noted earlier in this module, when an individual expects something to happen, she is more likely to notice it. This gives rise to what is known as observer bias. A classic example is provided by a study of college students observing flatworm behavior by Cordaro and Ison (1963). Student observers were to record how many head turns and body contractions were made by two groups of flatworms. Although there were no differences between the two groups, the observers were led to believe group A would have higher rates of both turning over and contracting than group B. The observers ended up recording twice as many head turns and three times as many body contractions for group A. Since there was no reason to expect a large difference in the actual behaviors, the results appear to be due to the expectations of the observers. Such biasing is typically not deliberate --- the observers may be trying to honestly report what they observer, but processes unknown to them are influencing their behavior.
...
This module introduces experiments as the best source of evidence about causal connections between variables. We assume that you already know something about causality, how to diagram causal connections, and the dangers of relying on correlational evidence for causation. In a well-designed experiment, the only plausible explanation for two variables varying in tandem is that they are causally connected. Alternative explanations---those based on the effects of other variables---have been ruled out by these hallmarks of experimental design:
 • an independent variable---the suspected cause---is manipulated
 • a dependent variable---the suspected effect---is carefully measured
 • extraneous variables are controlled
If the independent variable is in fact causally related to the dependent variable, then altering the value of the independent variable will produce changes in the value of the dependent variable (although not necessarily in all circumstances). Moreover, these changes would not be expected if the causal connection did not exist.
...
Whenever you see the term independent variable used by itself, it is your responsibility to decide whether it is manipulated or measured. This will be important when you evaluate the outcome of the study and ask how well it supports the suspected causal connection.
Dependent variable
...
As graphed below, people in the hypothetical no-oatmeal world would have higher cholesterol on average than people in the hypothetical always-oatmeal world.
\includegraphics[keepaspectratio,width=\textwidth,height=0.75\textheight]{experimentHypotheticalPopulations.jpg}
...
Thus, researchers begin with a random (or representative) sample from the actual population, and divide that group into sub-samples. In one of the simplest experimental designs, the independent variable has one value that is thought to affect the dependent variable (often called the treatment condition) and a neutral value that is thought to have no effect (often called the control condition). The experimental manipulation involves delivering the value that is thought to affect the DV to one sample, known as the experimental or treatment group, and the neutral value to the other sample, known as the control group.
 • The experimental group receives the experimental treatment. It functions as a sample of the hypothetical population in which everyone received the experimental treatment
 • The control group acts as if it were a sample of the hypothetical population in which no one received the experimental treatment
...
A confounding variable is also called:
 • Confound
 • Confounded variable
Confounding variables can arise at many different points in the experiment. There may be differences between the subjects comprising the experimental and control group. For example, the experimental group subjects in the cholesterol study might be cat owners whereas the control group subject may be dog owners. Or the experimental group subjects may smoke while the control group subjects may be non-smokers. These variables on which subjects differ are known as subject variables.
There may also be factors in the experimental design that vary. For example, those eating oatmeal each day may also consume more milk if they regularly put milk on their oatmeal. And they may eat breakfast more regularly than those in the control group. These are known as procedural variables.

The challenge of discovering confounds
...
\includegraphics[width=86pt,height=58pt]{Cleverhans2.jpg}
...
How good is the experiment? assessing validity
...
Internal validity
The first form of validity looks inside the experiment to the design of the experiment. The central question of internal validity is this: Was the experiment designed so as to determine what was really happening?
Internal validity: the extent to which a study's design ensures that its result will be in accordance with reality. That is, if the suspected cause really is a cause, the predicted effect on the DV is confirmed; if not, the predicted effect on the DV is not confirmed. To ensure this, other possible causes must be well-controlled.
External validity
The second form, external validity, looks outside the experiment itself to what conclusions can be drawn from the resulting data.
...
What can undermine external validity?
...
Scientific claims, as we have observed, are typically general claims --- claims about all comets or all cases of cell division. The collection of all instances about which one is making a claim is call a population. The term population is most commonly used for groups of people --- the population of France or of New York City. But as scientists use the term, any group of entities treated as individuals in a scientific study counts as a population. The entities might be individual people who meet a certain condition --- the population of citizens of the United States who were born in other countries. But the entities might be themselves groups of people --- the population of Girl Scout Troops in California. Or the entities might not involve people at all --- the population might be comprised of atoms at a given energy level.
If a given population is sufficiently small, we might be able to evaluate claims about it by examining all its members. For example, the population of interest might be living former Presidents of the United States. If we were interested in whether all these individuals were millionaires, we would merely need to examine each of them and evaluate whether each was a millionaire. But most claims in science are about much larger populations, even populations whose number of members is not known (think about the population of atoms at a given energy level at this moment of time). In these cases, scientists examine only some members of the population and try to justify claims about the whole population. The individuals examined comprise a sample.
If scientists have sufficient confidence that each member of the population is identical and will behave identically under the circumstances of interest, then they may elect to examine only one or a few instances of a population and feel confident in generalizing to the whole population. For example, molecules of ordinary hydrogen gas are assumed to be nearly identical and to exhibit the same properties when examined under the same conditions. Likewise, human liver cells are assumed to be pretty much identical in terms of the biochemical reactions they carry out. Scientists interested in understanding these entities may elect only examine a small sample.
In other cases in which the population is assumed to be variable, scientists need to examine a sufficiently large sample to estimate the distribution in the entire population. As we will see shortly, the size of the sample that must be considered to evaluate a particular claim about the population can be specified precisely.
The goal in drawing a sample is to be able to establish conclusions about the actual population. Most commonly, one is interesting in using a sample to estimate the mean in the actual population.
The following applet will provide you an appreciation of the sampling process. When you bring up the applet, you will see four graphs. The top represents the real population. In the initial settings, it has a normal distribution. The second graph is used to show the results of a single sample, whereas the bottom two graphs enable you to see how two statistical measures of the sample change over successive samples. As initially configured, the drop-down boxes by the third graph indicate that it will be recording the mean in each sample with a sample size (n) = 5. By selecting the Animate Sample button you will begin the process of randomly selected five items from the population (little black boxes will drop down in the second graph) and when that is complete, the mean of the sample will be calculated and a blue box will drop into the third graph.
You can select Animate Sample multiple times and successive samples will be drawn and their means added to the third graph. You can also select the 5 Samples, 1,000 Samples, and 10,000 Samples boxes and collect a large number of samples quickly. Notice how the values of the mean of the sample vary between samples. Try varying the size of the samples by altering the number in the box next to the right of the third graph, taking several samples of each size. How do larger and smaller sample sizes affect the stability of the means shown in the third graph?
Sampling Distribution
Click on this button to launch an applet that will illustrate the process of taking samples: Your browser either does not support JAVA or has JAVA disabled.
This applet was created by David M. Lane and is part of the Rice Virtual Lab in Statistics (http:\slash \slash www.ruf.rice.edu\slash \ensuremath{\sim}lane\slash rvls.html\slash ). We are grateful that he has made it publicly available.
As we have already noted, the median is an alternative measure of central tendency that is sometimes to be preferred to the mean. To see how means and medians compare, use the drop-down box for the fourth graph to select median and draw samples with n=5. Then raise the n for both of the lower graphs to 20, and draw more samples. How much agreement is there between the means and medians under these two conditions? Now use the drop-down box to the right of the first graph to select a skewed distribution for the actual population. Which measure, the mean or the median, tends to provide a clearer picture of the actual distribution?
You can also create your distribution by ``painting'' the top graph. Use your mouse to change the height of individual histogram bars, or, by clicking on one bar and dragging your mouse across other bars, paint the pattern of the histogram. Try creating different distributions of your own (try out, for example, a bimodal distribution) and take various samples to develop your understanding of the sampling process.
...
\end{apatextbox}
 %\captionof{InfoBox}{The Nuremberg Code\label{sample:nuremberg}}
...
The APA Standards were first published in 1953. They were revised in 1959, 1963 and 1968. The 1968 standards, which should guide the research committee in determining which research programs should be allowed to move forward, are attached in \fullref{app: APAEthics}.
